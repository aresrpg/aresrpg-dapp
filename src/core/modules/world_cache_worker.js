...
const fill_chunk = (chunk, block_pos, ground_type, buffer_over = []) => {
...
const add_edges_blocks = chunk => {
  ...
  for (let i = 0; i < patch_size; i++) {
    ...
    edges.forEach(edge => {
      ...
      fill_chunk(chunk, block_local_pos, block_data.type)
      // else console.log('missing block: ', edge.pos)
    })
  }
}

const add_ground_blocks = (patch, chunk) => {
  ...
  while (!res.done) {
    ...
    fill_chunk(chunk, block_pos, block_type)
    res = iter.next()
  }
}

const add_entities_blocks = (patch, chunk) => {
  // patch.spawned
  const buff_iter = patch.overBlocksIter()
  for (const blk of buff_iter) {
    blk.localPos.x += 1
    blk.localPos.z += 1
    fill_chunk(chunk, blk.localPos, blk.type, blk.buffer)
  }
}
...
const get_chunk = (block_start, block_end) => {
  ...
  if (patch) {
    add_ground_blocks(patch, chunk)
    // process entities
    add_entities_blocks(patch, chunk)
    // fill extra blocks at edges from adjacent patches
    add_edges_blocks(chunk)
    is_empty = false
  }
  return cache
}
...
addEventListener('message', ({ data: input }) => {
  ...
  switch (input.api) {
    ...
    case 'getChunk':
      ...
      break
    case 'updateCache': {
      const promise = update_cache(...input.args)
      if (promise) {
        promise.then(res => {
        ...
        output.data = { cacheRefreshed: false }
        postMessage(output)
      }
      break
    }
